    ...
    protected void enable() {
        ...
        if (lx != null && lx.getConfig().getBoolean("options.nerf-other-addons")) {
            Scheduler.run(() -> log(Level.WARNING,
                    "########################################################",
                    "LiteXpansion đã làm giảm hiệu suất máy phát trong addon này",
                    "Bạn có thể thay đổi cài đặt trong tệp cấu hình của LiteXpansion",
                    "Đặt 'nerf-other-addons' thành false trong 'options'",
                    "########################################################"
            ));
        }
